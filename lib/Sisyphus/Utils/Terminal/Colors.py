...
# -*- coding: utf-8 -*-
"""
Sisyphus/Utils/Terminal/Colors.py
...
def color(*args):
    """Convert a named color or hex value to a RGB tuple
        ...
        Traceback (most recent call last):
            ...
        ValueError: color() only allows 1 or 3 arguments
        >>> color("#f3g")
        Traceback (most recent call last):
            ...
    ...
    if type(arg) is str:
        if arg.lower() in named_colors.keys():
        ...
        if "," in arg:
            try:
                ...
                if type(expr) in (list, tuple) and len(expr)==3:
                    return color(*expr)
            except ValueError:
                pass # do nothing
        
        
        if arg.isnumeric() or (arg[:2] in ("0x", "0X", "0o", "0O", "0b", "0B")):
            expr = ast.literal_eval(arg)

        elif arg[0] == "#":
            expr = None
            if len(arg) == 4:
                hexstr = "0x" + "".join([x+x for x in arg[1:]])
            elif len(arg) == 7:
                hexstr = "0x" + "".join([x for x in arg[1:]])

            try:
                expr = ast.literal_eval(hexstr)
